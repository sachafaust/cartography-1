...
**Contrarian-review remediations (also implemented on this branch):**

An adversarial review of the three changes surfaced two real hazards, both now fixed and validated:

1. *Finding 3's memory cliff*: `DETACH DELETE` of 1000 nodes × N relationships holds 1000·N relationship deletions in one transaction. Measured on the repo's shipped 1 GB-heap settings: fine through node degree ~500, but at degree 1000 (≈1M relationship deletes per transaction) the cleanup **hard-failed** with `MemoryPoolOutOfMemoryError` — where `LIMIT 100` still worked. Remediations:
   - `GraphStatement._run_iterative` now **halves its batch size and retries** on `MemoryPoolOutOfMemoryError` (floor: 100, the old default, which has years of production safety). The exact scenario that hard-failed now completes with one halving. Non-memory transient errors are re-raised unchanged.
   - New **`--cleanup-batch-size`** config/CLI option (default 1000) threaded through `Config` → `GraphJob.from_node_schema`/`from_matchlink` defaults and applied over the values baked into the packaged JSON cleanup jobs, so operators can tune deterministically without forking.
   - Measured at 10 ms RTT (via a latency proxy): 20k deletes take 10.4 s at batch 100 vs 2.3 s at batch 1000 (**4.5x**, larger than the 2.2x localhost figure). Also noted honestly: the benefit is ~zero for node degree ≥ 100 and for no-op cleanups; the win is per-transaction overhead on the (common) low-degree case.
2. *Finding 2's staleness hazard*: the `ensure_indexes` cache was keyed by schema class only, so a long-running process syncing a second (or recreated) database would silently never create its indexes — and an unindexed `MERGE` load degrades catastrophically (a 20k-item load did not finish in 10 minutes vs ~4 s indexed). `Sync.run()` now **clears the cache at the start of every sync run**, bounding staleness to one run while keeping the within-run win (which is where the thousands of repeat calls occur). Measured at 10 ms RTT: memoization saves **157 ms per repeat `load()` call**.

Review notes that did not require code changes: the Identity Center matcher was re-verified on the *real* generated query (planner picks `NodeIndexSeekByRange` on `name` over the low-selectivity `path` index; 1000 permission sets vs 100k roles in 0.2–1.4 s), and two behavior notes belong in release notes — the prefix match no longer creates the old matcher's false-positive links for prefix-named permission sets (e.g. "Admin" vs "AdminPlus" roles; stale links are cleaned up on the next sync), and role matching now requires `AWSRole.path`, which any iam sync from the last several years populates.
